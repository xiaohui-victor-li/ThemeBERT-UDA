...
class KeyphraseCountVectorizer(_KeyphraseVectorizerMixin, BaseEstimator):
    ...
    def __init__(self, spacy_pipeline: str = 'en_core_web_md', pos_pattern: str = '<J.*>*<N.*>+',
    ...
    def fit(self, raw_documents: List[str]) -> object:
        ...
        #### * Case of Empty Keyphrases * ####
        if len(self.keyphrases)==0:
            print("Empty keyphrases. Perhaps the documents do not contain keyphrases that match the 'pos_pattern' parameter, only contain stop words.")
            return None
        ##########################################      
    ...
    def get_feature_names_out(self) -> np.array(str):
        ...
        count_vector = CountVectorizer(vocabulary=self.keyphrases, ngram_range=(self.min_n_gram_length, self.max_n_gram_length),
                                           lowercase=self.lowercase, binary=self.binary, dtype=self.dtype)

        return count_vector.get_feature_names_out()
