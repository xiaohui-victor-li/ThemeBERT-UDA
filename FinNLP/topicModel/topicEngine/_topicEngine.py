...
from ._utils import NLPLogger, check_documents_type, check_embeddings_shape, check_is_fitted
...
logger = NLPLogger("WARNING")

TOPIC_META_PATH = None

class TopicEngine:
    """Topic Engine is a topic modeling class that leverages contextual embeddings and
    ...
    topic_model = TopicEngine(embedding_model=sentence_model)
    ```
    """
    def __init__(self,
                 ...
                 topic_meta_path: str = None,
                 verbose: bool = False,
                 ):
        """TopicEngine backend class

        Input Args:
            language: The main language used in your documents. 
                      Select "multilingual" to load in a sentence-tranformers model that supports 50+ languages.
            top_n_words: The number of words per topic to extract. Setting this
                         too high can negatively impact topic embeddings.
            ...
            verbose: Changes the verbosity of the model,
        ...
        if embedding_model is None or isinstance(embedding_model, str):
            self.embedding_model = select_backend(embedding_model, language=self.language)
        else: # CASE of assign the model
            self.embedding_model = embedding_model

        # Vectorizer
        self.n_gram_range = n_gram_range

        self.vectorizer_model = vectorizer_model or CountVectorizer(ngram_range=self.n_gram_range,stop_words=self.language)
        ...
        # load the prestored topic
        self.topic_meta_path = topic_meta_path
        if self.topic_meta_path is not None:
            # metadata
            topic_meta_table = pd.read_csv(self.topic_meta_path)
            self.topic_meta = topic_meta_table.loc[topic_meta_table['Raw_Topic'] != "Filter"]
            self.topic_effective_ids = self.topic_meta['Topic_ID'].values+1  # effective topics identifed by meta table
            self.topic_cutoff_delta = self.topic_meta['Threshold'].values.reshape(1, -1)
            self.topic_transformer = None    # scikit-learn fitted quintile transformer 
    ...
    def fit(self,
        ...
        ```python
        # Create topic model
        topic_model = TopicEngine().fit(docs, embeddings)
        ```
        """
        self.fit_transform(documents, embeddings, umap_embeddings, y)
        return self
    ...
    def fit_transform(self,
        ...
        ## Customized guided with prompts
    ...
    def compute_theme_exposure(self,
                               embeddings: np.ndarray = None,
                               documents: Union[str, List[str]] = None,
                               flag_cutoff=True):
        """ Render the thematic exposure (0,1) to a predefined list of theme (self.topic_meta)
        """
        ## => process the input text corpus into embeddings list
        if isinstance(documents, str):
            documents = [documents]
        if embeddings is None:
            embeddings = self._extract_embeddings(documents,
        ...
        if self.topic_effective_ids is not None:
            topic_effective_embeddings = self.topic_embeddings[self.topic_effective_ids]
        else:
            topic_effective_embeddings = self.topic_embeddings[1:]
        doc_cosim_mat = cosine_similarity(embeddings, topic_effective_embeddings)

        if self.topic_transformer is not None:
            # uniform transformed similarity score
            doc_cosim_mat_perc =  self.topic_transformer.transform(doc_cosim_mat)
        else:
            doc_cosim_mat_perc =  doc_cosim_mat 

        if flag_cutoff:  # filter the small exposure noise
            exposure_delta_mat = self.topic_cutoff_delta.repeat(sample_size, axis=0)
            bool_filter = (doc_cosim_mat<=exposure_delta_mat)
            doc_cosim_mat_perc[bool_filter] = 0

        ## => wrap into output exposure DF
        doc_topic_exposure = pd.DataFrame(doc_cosim_mat_perc).T

        if self.topic_meta is not None:
            doc_topic_exposure = doc_topic_exposure.join(self.topic_meta[['Theme_Category','Theme', 'Raw_Topic']])
            output_topic_exposure = doc_topic_exposure.set_index(['Theme_Category','Theme', 'Raw_Topic'])
        else:
            output_topic_exposure = doc_topic_exposure.T

        return output_topic_exposure
    ...
    ## Topic Update func
    def update_topic_engine(self, documents: pd.DataFrame):
        """ Update topics from the classified dataframe
        ...
        """
        #### => Update the topic sizes
        self._update_topic_size(documents)

        #### => Update the topic dictionary & c_tf_idf matrix
    ...
    ###### ✨ _Foundational Topic Top Words Extraction Function  ######
    def _load_existing_topics(self, topic_dict=None, topic_path=None):
        """load the existing topic data"""
        if topic_path is not None:
            ## load the topic dict
            with open(os.path.join(topic_path), 'r') as fp:
                old_topic_dict = json.load(fp)
            topic_dict = {}
            for k,val in old_topic_dict.items():
                topic_dict[int(k)] = val
        elif topic_dict is not None:
            # update the topic 
            self.topics = topic_dict
            # update the topic embeddings
            self._create_topic_vectors()    # 📑: create the self.topic_embeddings obj.  
    ...
    def _create_topic_vectors(self):
        ...
        🗺️: Start by creating embeddings out of the topic representation. This
        ...
        if self.embedding_model is not None:
            ...
            for i, topic in enumerate(topic_list):
                ...
                topic_words = [val[0] for val in self.get_topic(topic)]
                word_embeddings = self._extract_embeddings(topic_words, method="word",verbose=False)

                ## Extract word importance score as weight
    ...
    def _guided_topic_modeling(self, embeddings: np.ndarray) -> Tuple[List[int], np.array]:
        ...
        for seed_topic in range(len(seed_topic_list)):
            # 🔍 query the docs that are relevant to the seeded topic
    ...
    ### ✨ Core Topic Top Words Extraction Function 
    def _c_tf_idf(self, documents_per_topic: pd.DataFrame, m: int, fit: bool = True) -> Tuple[csr_matrix, List[str]]:
        """ Calculate a class-based TF-IDF where m is the number of total documents.
    ...
    def __str__(self):
        ...
        return f"TopicEngine({parameters[:-2]})"
